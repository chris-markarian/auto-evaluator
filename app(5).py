...
# Manual paste toggle
manual_paste = st.checkbox("Paste listing content manually (better accuracy)")
...
# If manual paste is checked, show paste area immediately
if manual_paste:
    st.session_state.show_paste = True

# Handle submission
if submit_clicked and url and not manual_paste:
...
# Show paste area if auto-fetch failed or manual paste selected
if st.session_state.show_paste:
    if manual_paste:
        st.info("📋 Paste the listing content below for better accuracy.")
    else:
    ...
    listing_text = st.text_area(
        ...
        placeholder="Go to the listing page, Select All (Ctrl+A), Copy (Ctrl+C), then paste here...",
        height=200,
